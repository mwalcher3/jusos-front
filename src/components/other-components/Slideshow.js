...
const Slideshow = ({ data, smallScreenData }) => {
  ...
  var carouselSmallSettings = {
    ...carouselSettings,
    ...{ dataSource: smallScreenImageSource, length: smallScreenImageSource.length },
  };

  if (data.data.length == 2) {
    carouselSettings.dots = false;
  }
  if (smallScreenData.data.length == 2) {
    carouselSmallSettings.dots = false;
  }
