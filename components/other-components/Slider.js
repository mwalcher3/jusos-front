...
const Slider = ({data, instaData}) => {  
    ...
    const dataArray=[{title: "Aktuelles", text: 
      (instaData && instaData.data && instaData.data.length()>0)? instaData.data[0].caption: "", link:"/aktuelles"}]
     ...
     const {access, endpoint}= relation
      ...
      access.data.map((item)=> {
        if(endpoint=="/termine"){
          ...
          dataArray.push({title: item.attributes.title, text: item.attributes.text, link: endpoint, date: formatedDate, location:item.attributes.location})
        }
        else{
   ...
   data.quotes.map((quote)=>{
    dataArray.push({title: quote.title, text: quote.text, link: quote.link, author: quote.author})
       ...
       {dataArray.map((item, index)=>{
           ...
           <div className={
            ...
            slidercss.none} 
              key={index}
              >
                <div ref={el => sliderRef.current[index] = el}  className={item.author? "": slidercss.sliderboxes}>
                  <h3><ReactMarkdown rehypePlugins={[rehypeRaw]}>{item.title? item.title:<></>}</ReactMarkdown></h3>
               <ReactMarkdown rehypePlugins={[rehypeRaw]}>{item.text}</ReactMarkdown>
               {item.link=="/termine"?<i><div>{item.date}</div><div>{item.location}</div></i>:<></>}
               {item.author? <i><div>{item.author}</div></i>: <></>}
               </div>  
               {
                item.link?
